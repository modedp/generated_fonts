...
function customFont.new(fontName, class, isButton, style)
	local self = event.new {}; style = style or nil
	...
	if style then
		...
		propertyobjects["Style"].Value = style
	end
	drawText();
...
for _, class in next, {"TextLabel", "TextBox", "TextButton", "TextReplace"} do
	module[string.sub(class, 5)] = function(fontName, child, other)
		return customFont.new(fontName, class == "TextReplace" and child or class, class == "TextButton" or (class == "TextReplace" and child:IsA("TextButton")),other);
	end;
end;
